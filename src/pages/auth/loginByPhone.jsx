...
function PhoneLogin() {
  ...
  const OnChange = (e) => {
    SetForm({ ...Form, [e.target.name]: e.target.value });
         ...
         useEffect (() => {
        document.title = "تسجيل الدخول بالهاتف - نظام أبو الدهب";
      }, []);
  ...
  const LoginProcess = async () => {
    ...
    try {
      const Res = await api.post("/users/phoneLogin", {
        phone: Form.phone,
      ...
      if (Data.accessToken) {
        ...
        role === "admin" ? Navigate("/admin_dashboard") : Navigate("/");
      }
    } catch (err) {
      ...
      <div className="w-full lg:w-1/2 flex flex-col items-center justify-center p-8 md:p-16 bg-[#f8fafc]">
        ...
        <div className="w-full max-w-[550px]">
          ...
          <div className="mb-12 text-right">
            ...
            <h1 className="text-4xl font-extrabold text-[#0f172a] mb-4">تسجيل الدخول بالهاتف</h1>
            <p className="text-xl text-gray-500">أهلاً بك! استخدم رقم هاتفك للوصول لحسابك.</p>
          </div>
          ...
          <div className="space-y-8">
            ...
            <div className="pt-6 text-right pr-2 flex flex-col gap-3 items-center justify-center md:flex-row md:justify-between">
              <p className="text-lg text-gray-500">
                  الدخول عبر البريد <span onClick={() => Navigate("/تسجيل_الدخول")} className="text-[#0284c7] font-bold cursor-pointer hover:underline">اضغط هنا</span>
              </p>

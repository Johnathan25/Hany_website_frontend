...
const StatusBadge = ({ status }) => {
  const statusMap = {
    ...
    paid: { label: "مدفوع ومؤكد", color: "bg-emerald-50 text-emerald-700 border-emerald-200" },
...
export default function InventionsInvoicesRecords() {
  ...
  return (
    <div dir="rtl" className="space-y-6">
            ...
            <table className="w-full text-right text-xs">
              <thead className="bg-slate-50 text-slate-500 border-b border-slate-200 font-semibold">
                <tr>
                  ...
                  <th className="py-3 px-4">بوابة الدفع</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {payments.map((p) => {
                  const clientName =
                    p.name ||
                    p.gatewayResponse?.data?.metaData?.name ||
                    p.customer?.username ||
                    "عميل غير محدد";

                  const clientPhone =
                    p.phone ||
                    p.gatewayResponse?.data?.metaData?.phone ||
                    p.customer?.phone ||
                    "---";
                  ...
                  const invNum =
                    p.invoiceNumber ||
                    p.reference ||
                    p.gatewayResponse?.data?.merchantOrderId ||
                    `#${p._id?.slice(-6)?.toUpperCase()}`;

                  const amount = Number(p.amount ?? p.gatewayResponse?.data?.amount ?? 0);
                  const currency = p.currency || p.gatewayResponse?.data?.currency || "EGP";
                  ...
                  return (
                    <tr key={p._id} className="hover:bg-slate-50/70 transition-colors">
                      ...
                      {/* Provider */}
                      <td className="py-3.5 px-4 whitespace-nowrap">
                        <span className="font-mono text-slate-600 uppercase text-[11px] font-semibold bg-slate-50 px-2 py-0.5 rounded border border-slate-200">
                          {p.provider || p.gatewayResponse?.platform || "kashier"}
                        </span>
                      </td>
      ...
      {/* ========================================================= */}
      {/* نافذة فحص وتفاصيل الفاتورة الإلكترونية الكاملة               */}
      {/* ========================================================= */}
      {selectedPayment && (() => {
        const gwData = selectedPayment.gatewayResponse?.data || {};
        const meta = gwData.metaData || {};
        const cardInfo =
          gwData.card?.cardInfo ||
          gwData.sourceOfFunds?.cardInfo ||
          {};
        ...
        const invNumber =
          ...
          gwData.merchantOrderId ||
          `#${selectedPayment._id?.slice(-6)?.toUpperCase()}`;

        const amount = Number(selectedPayment.amount ?? gwData.amount ?? 0);
        const currency = selectedPayment.currency || gwData.currency || "EGP";
        const transactionId = selectedPayment.transactionId || gwData.transactionId || "---";
        const kashierOrderId = gwData.kashierOrderId || "---";
        const orderReference = gwData.orderReference || "---";
        const sessionId = selectedPayment.sessionId || "---";
        const paidDate = selectedPayment.paidAt || selectedPayment.updatedAt || selectedPayment.createdAt;
            ...
            <div className="bg-white rounded-3xl max-w-xl w-full overflow-hidden shadow-2xl border border-slate-100 max-h-[92vh] flex flex-col animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-slate-900">
                      فاتورة رقم: {invNumber}
                    </h3>
                    <span className="text-[10px] text-slate-400 font-mono">
                      Doc ID: {selectedPayment._id}
                    </span>
                  </div>
                </div>
                <button
              ...
              <div className="p-6 space-y-5 overflow-y-auto">
                ...
                <div className="grid grid-cols-2 gap-3 p-4 bg-blue-50/60 border border-blue-100 rounded-2xl">
                  <div>
                    <span className="block text-[10px] text-blue-600 font-bold mb-0.5">المبلغ المحصل فعلياً</span>
                    ...
                    <span className="block text-[10px] text-emerald-600 font-semibold mt-0.5">
                      {selectedPayment.status === "paid" ? "تم التحصيل بنجاح" : "بانتظار السداد"}
                    </span>
                  </div>

                  <div>
                    <span className="block text-[10px] text-slate-500 font-bold mb-0.5">نوع الدفعة البنكية</span>
                    <span className="text-sm font-bold text-slate-800 block mt-1">
                      {selectedPayment.paymentType === "deposit" ? "عربون حجز مقدم" : "سداد كامل القيمة"}
                    </span>
                    <span className="text-[10px] text-slate-400 font-mono">
                      بوابة: {selectedPayment.provider || "kashier"}
                    </span>
                  </div>
                </div>

                {/* بيانات البطاقة البنكية المستخدمة إن وجدت في استجابة كاشير */}
                {cardInfo.maskedCard && (
                  <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs space-y-2">
                    <div className="flex items-center gap-2 text-slate-700 font-bold mb-1">
                      ...
                      <span>بيانات بطاقة الدفع (Visa / Mastercard)</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-slate-600 font-mono">
                      <div>
                        <span className="text-slate-400 text-[10px] block">رقم الكارت:</span>
                        <span className="font-bold text-slate-900">{cardInfo.maskedCard}</span>
                      </div>
                      <div>
                        <span className="text-slate-400 text-[10px] block">نوع الكارت:</span>
                        <span className="font-bold text-slate-900">{cardInfo.cardBrand || "بطاقة ائتمان"}</span>
                      </div>
                      {cardInfo.cardHolderName && (
                        <div className="col-span-2">
                          <span className="text-slate-400 text-[10px] block">اسم حامل البطاقة:</span>
                          <span className="text-slate-800 font-sans font-semibold">{cardInfo.cardHolderName}</span>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs">
                    ...
                    <div>
                      ...
                      <span className="font-mono font-semibold text-slate-800">{clientPhone}</span>
                    </div>
                    <div className="sm:col-span-2">
                      <span className="block text-slate-400 text-[10px]">البريد الإلكتروني</span>
                      <span className="font-mono text-slate-800">{clientEmail}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs space-y-2 font-mono">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-slate-700">{orderReference}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-slate-400">Kashier Order ID:</span>
                      <span className="text-slate-700 truncate max-w-[200px]" title={kashierOrderId}>
                        {kashierOrderId}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-slate-400">Session ID:</span>
                      <span className="text-slate-700 truncate max-w-[200px]" title={sessionId}>
                        {sessionId}
                      </span>
                    </div>
                    {paidDate && (
                      <div className="flex justify-between text-emerald-700 font-bold pt-1.5 border-t border-slate-200 text-[11px]">
                        <span>تاريخ تأكيد السداد:</span>
